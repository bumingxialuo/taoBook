//
//  HEEBookRankingViewController.m
//  taoBook
//
...
@interface HEEBookRankingViewController ()<UIScrollViewDelegate,HEEBookRankingTVCDelegate>
...
@property (nonatomic, weak) HEEBookRankingTVC *allBookVC;                           //综合
@property (nonatomic, weak) HEEBookRankingTVC *rekeaseLatestVC;                     //交易量
@property (nonatomic, weak) HEEBookRankingTVC *precedenceLowVC;                     //需求量
@end

@implementation HEEBookRankingViewController

//懒加载属性
- (NSArray *)titleArray {
    if (!_titleArray) {
        _titleArray = @[@"综合", @"交易量", @"需求量"];
    }
    return  _titleArray;
}
...
- (void)layoutNav {
    self.title = [NSString stringWithFormat:@"好书榜"];
